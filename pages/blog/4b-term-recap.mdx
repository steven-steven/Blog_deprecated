...
export const meta = {  date: '2021-04-10',  title: 'Closure (a 4B term Recap)'  };
...
Flamegraph is just a top-tier profiling tool ✨. It spot-on identifies where to improve the code for performance, rather than simply 'guessing'. In the assignment for example, initializing iterators and backoffs from message-passing channels are way more expensive compared to the I/O (who would've known if it wasn't for Flamegraphs!). It was also a pretty fun activity to try different methods and see if it could improve the runitme (or slow it down 😢). 
...
CUDA code is a new paradigm to me. The global cuda function called by the host will be executed by multiple independent cuda threads. From what I understood ([stackoverflow](https://stackoverflow.com/questions/14093692/whats-the-difference-between-cuda-shared-and-global-memory)), the pointer arguments copied from the host will be located in device as a 'global memory' and shared amongst all cuda threads. So coding the cuda operations I make sure not to write to the same memory output (e.g. each thread writes to its dedicated indices on an output matrix). To tell which thread does which work, normally involves using the set of indexes uniquely available to the current thread (threadIdx.x, blockIdx.x, blockDim.x, etc). The indices come from the landscape of the cuda threads, which are partitioned into 2D constructs called blocks and each blocks are partitioned in a 2D construct called grid. We are free to define the block & grid sizes in the host code (within limits). In particular this diagram below helps me a lot when visualizing the gpu thread landscape: (image source: Nvidia docs). In that example, our grid size is (2x3), block size is (3x4), and thus 12 * 6 = 72 gpu threads in total.
...
When you can't speed up, just spend 💰 and scale. However testing could now become harder (e.g. test multiple machines), and sometimes you'd have to simulate / put it under pressure to begin seeing things blow up 💣. Potential culprit could be CPU, memory usage, network, etc. You gotta figure out the what limits your system 🕵️‍♂️🔎.
...
There was a few notes on Ops that I think was worth the reminder. One of which is to treat 'configuration as code' - to avoid human mistakes of installing certaind dependencies, setting up, and you get to version control it. Containers are one approach but you need to remember to upgrade things frequently (which is not enforced) - for security reasons.
...
- Canny Edge Detection: A general method to get clean edges
...
- In the assignment, we detect road lines from a video. The [starter code is actually from Udacity](https://github.com/udacity/CarND-LaneLines-P1). After masking a trapezoid region of interest, implementing Canny Edge to obtain the edges, I used 'Hough Transform' to parameterize the lines (map the x,y pixels to a Hough space and vote for the max overlaps of d, θ). Once we get the line equation (d = xcos(theta) - ysin(theta)), we draw the solid lanes for that frame.

- Generally, hand-coding algorithms with first principles is good enough for low-level classification (ie. edges/dots). But for high-level semantic labels (ie. bicycle, cats), using ML makes it possible to identify the variations by learning the solution from 'data'. ML can also fail in spectacular ways, which is scary especially for these critical systems.

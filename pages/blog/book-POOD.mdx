...
Practical Object-Oriented Design, by Sandi Metz, is a book I recently read. In the book Ruby was merely just a tool to explain the concepts of designing a program in OOP. The book went through some iterative thought process of how one would approach writing 'changeable code' - one that is adaptable for future changes and reduce cost of maintainance.

I wanted this post to be a pictorial collection of my notes about some difficult concepts. Some of which might be applicable in the near future. But by no means is this a summary.

### Single Responsibility
- Class and methods should do one thing. You could summarize the method/class's purpose in one sentence. Small methods are easier to maintain, obvious, encourage reuse.
...
## Managing Dependency
- Dependency: the need to know other class's name, method, arguments, order of arguments. A change in a class requires change in its dependents. This is costly.
- The direction of dependency could go in either direction. As a rule of thumb, always depend on more stable things (won't change often). I'd imagine a dependency tree where the root more stable then it's dependents. E.g. abstract class is more stable than concrete ones. Concrete class shouldn't have many dependents (other class calling it).
...
- should only test public interface of the class.
...
- calling super in Ruby, calls parent method (with the same name). If no argument specified, by default the overrided pass all its arguments to parent.
- The abstract class is a repo of common behavior. Can't be instantiated.
- Template method pattern: superclass sends the messages to acquire the specializations of it's subclass. Superclass must define fallback method (a template - a HOOK) if the subclass doesn't implement it.
  - thus, subclass don't have to know how to interact with superclass. It doesn't need to send 'super' (calling super requires subclass to know superclass's implementation, which is bad / tight-coupling).
...
- Test public interface: is a test of state (make assertion about return values)
- Testing the 2 types of outgoing message.
  1. Commands - has side-effect (eg. file I/O, DB). Is the sender's responsibility to prove that it's properly sent. Is a test of behavior.
      - assert # times, with what arguments the message is sent.
...
- Testing an expensive role
  ...
  - stubs is a dummy object that implements the same interface. However, what if the role changes? then the test double is forced to change. How is this enforced (to prevent false positive)? 
  - Test module class: Use shareable tests! Easy way to test every object that plays the role
...
- You could define shared module (ie. AnimalInterfaceTest) to test all subclass that they responds_to the shared public abstract methods.
- You could define shared module (ie. AnimalSubclassTest) to test all subclass that tehy respond_to the required abstract methods to override (note: if not overriden, the abstract class should throw error).

![Test interface](/static/blogAssets/book-POOD/testInterface.png)

### And that is all!

Learned stuff that seems pretty important but is often ignored. I'm hoping reading this would help me be more conscious of design decisions in every code I touch.
